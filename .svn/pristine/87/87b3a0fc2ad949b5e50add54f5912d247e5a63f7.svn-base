...
{
    CLSHAccountIncomeAndExpendDetailModel *incomeAndExpendDetailModel;  ///<收支详情数据模型
...
- (void)loadData
{
    incomeAndExpendDetailModel = [[CLSHAccountIncomeAndExpendDetailModel alloc] init];
    params = [NSMutableDictionary dictionary];
    //传入对应的ID
    params[@"id"] = self.typeID;
    [incomeAndExpendDetailModel fetchAccountIncomeAndExpendDetailData:params callBack:^(BOOL isSuccess, id  _Nonnull result) {
        if (isSuccess) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        firstCell.incomeAndExpendDetailModel = incomeAndExpendDetailModel;
