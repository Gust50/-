...
-(UIImageView *)backgroundView{
    if (!_backgroundView) {
        ...
        _backgroundView.image = [UIImage imageNamed:@"StoreTop"];
...
//    }];
    ...
    [_purchaseCount mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(120*pro, 18));
    }];
...
- (void)setNeighbourhoodMerchantTopModel:(CLSHNeighborhoodMerchantLeftModel *)NeighbourhoodMerchantTopModel{
    ...
    NSString *str = [NSString stringWithFormat:@"凡在本店购买，全场%@",NeighbourhoodMerchantTopModel.giftTitle];
    _discountLabel.attributedText = [NSString attriTipBySpecialSymbolWithTipStr:str normalColor:[UIColor  whiteColor] highColor:[UIColor whiteColor] normalSize:10*pro highSize:10*pro];
