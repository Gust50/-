...
- (UIButton *)selectBtn{
    ...
    if (!_selectBtn) {
        ...
        _selectBtn.titleLabel.font = [UIFont systemFontOfSize:14*pro];
...
- (UILabel *)sumMoneyLabel{
    ...
    if (!_sumMoneyLabel) {
        ...
        _sumMoneyLabel.font = [UIFont systemFontOfSize:14*pro];
...
- (UIButton *)sureSettle{
    ...
    if (!_sureSettle) {
        ...
        [_sureSettle setTitle:@"确认取消" forState:(UIControlStateNormal)];
        _sureSettle.titleLabel.font = [UIFont systemFontOfSize:12*pro];
        _sureSettle.backgroundColor = systemColor;
        _sureSettle.layer.masksToBounds = YES;
        _sureSettle.layer.cornerRadius = 10*pro;
...
- (void)updateConstraints{
    ...
    [_sumMoneyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_selectBtn.mas_right).with.offset(8*pro);
        make.bottom.equalTo(weakSelf.mas_bottom);
        make.top.equalTo(weakSelf.mas_top);
