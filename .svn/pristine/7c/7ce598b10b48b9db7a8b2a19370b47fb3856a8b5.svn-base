...
#define kTopViewHeight         190*pro
#define kMiddleViewHeight      40*pro
...
@interface CLSHNeighbourhoodMerchantVC ()<UITableViewDataSource,UITableViewDelegate,KBSegmentViewDelegate,NeightbourhoodShopRightCellDelegate>{
    ...
    CLSHRemarkAnswerView *remarkAnswerView;///<评论回复视图
...
- (UITableView *)commentTableView{
    ...
    if (!_commentTableView) {
        _commentTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, kTopViewHeight+kMiddleViewHeight, SCREENWIDTH, SCREENHEIGHT-kTopViewHeight-kMiddleViewHeight) style:(UITableViewStyleGrouped)];
...
{
    if(tableView == _commentTableView){
        remarkListModel = remarkArr[section];
        if (remarkListModel.answer.content) {
            CGSize size = [NSString sizeWithStr:remarkListModel.answer.content font:[UIFont systemFontOfSize:12*pro] width:SCREENWIDTH-40*pro];
            return 50*pro+size.height;
        }else
        {
            return CGFLOAT_MIN;
        }
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    if(tableView == _commentTableView){
        remarkListModel = remarkArr[section];
        CGSize size = [NSString sizeWithStr:remarkListModel.answer.content font:[UIFont systemFontOfSize:12*pro] width:SCREENWIDTH-40*pro];
        if (remarkListModel.answer.content != nil) {
            UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, 50*pro+size.height)];
            view.backgroundColor = [UIColor whiteColor];
            remarkAnswerView = [[CLSHRemarkAnswerView alloc]initWithFrame:CGRectMake(10*pro, 5*pro, SCREENWIDTH-20*pro, 40*pro+size.height)];
            remarkAnswerView.remark = remarkListModel.answer.content;
            remarkAnswerView.remarkTime = remarkListModel.answer.answerTime;
            remarkAnswerView.remarkName = remarkListModel.answer.name;
            remarkAnswerView.layer.cornerRadius = 5*pro;
            remarkAnswerView.layer.masksToBounds = YES;
            remarkAnswerView.backgroundColor = RGBColor(239, 239, 242);
            remarkAnswerView.layer.borderColor = RGBAColor(0, 0, 0, 0.3).CGColor;
            remarkAnswerView.layer.borderWidth = 1;
            [view addSubview:remarkAnswerView];
            return view;
        }
    }
    return nil;
