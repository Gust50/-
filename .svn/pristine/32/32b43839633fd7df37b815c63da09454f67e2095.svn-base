    ...
    failure:(void (^)(NSError *))failure
{
    ...
    [manager POST:url parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (baseModel.code==ResponseSuccess) {
            ...
            [ShareApp backToRootTabbar];
        }else if (baseModel.code==ResponseUpdateKey){
            [FetchAppPublicKeyModel shareAppPublicKeyManager].publicKey=nil;
             [ShareApp updatePublickKey];
        }
        else{
            success(responseObject);
...
//    [request addValue:@"3.0" forHTTPHeaderField:@"app_version"];
    [request setValue:@"3.0" forHTTPHeaderField:@"app_version"];
