...
@interface CLSHMerchantWalletCell ()
...
@implementation CLSHMerchantWalletCell
...
-(UIButton *)getWallet{
    if (!_getWallet) {
        ...
        [_getWallet addTarget:self action:@selector(searchAdvertise) forControlEvents:UIControlEventTouchUpInside];
    }
    return _getWallet;
}
...
{
    if (self.searchMerchantAdvertisBlock) {
        self.searchMerchantAdvertisBlock();
    }
}

//商家红包
-(void)setMerchantWalletListModel:(CLSHMerchantWalletListModel *)merchantWalletListModel
{
    _merchantWalletListModel = merchantWalletListModel;
    _walletTypeName.text = merchantWalletListModel.shopName;
    _walletDescribe.text = merchantWalletListModel.introduction;
    if (merchantWalletListModel.state) {
        _getWallet.backgroundColor = RGBColor(196, 197, 198);
        [_getWallet setTitle:@"已领取" forState:UIControlStateNormal];
