...
- (void)viewDidLoad {
    ...
    [self.navigationItem.rightBarButtonItem setTitleTextAttributes :[ NSDictionary dictionaryWithObjectsAndKeys :[ UIFont fontWithName : @"Helvetica" size : 16.0 ], NSFontAttributeName ,[UIColor whiteColor ], NSForegroundColorAttributeName ,nil ]forState : UIControlStateNormal ];
...
- (void)createUI{
    ...
    self.cv = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 65, WIDTH, HEIGHT-105)collectionViewLayout:fl];
...
- (UICollectionViewCell*)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.btnAction = ^{
        ...
        if (cell.btn.selected == NO) {
            ...
            [btn setBackgroundImage:[UIImage imageNamed:@"p.png"] forState:UIControlStateNormal];
           ...
           // NSLog(@"%ld",self.selectArr.count);
            ...
            [btn setBackgroundImage:[UIImage imageNamed:@"p1.png"] forState:UIControlStateNormal];
        ...
        for (NSInteger i = 0; i < assetsFetchResults.count; i++) {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                ...
                if (self.arr.count == assetsFetchResults.count){
                    ...
                    //reloadData为一大坑，故改为[self createUI]
                    //[self.cv reloadData];
                    [self createUI];
                }
            }];
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
            ...
            UIImage * image = [UIImage imageWithData:imageData];
                ...
                if (self.arr.count == fetchResult.count) {
                    ...
                    //reloadData为一大坑，改为[self createUI]
                    //[self.cv reloadData];
                    [self createUI];
...
- (void)popto{
    ...
    dict[@"arr"] = self.selectArr;
    dict[@"sec"] = @(_sec);
    self.popSelectArr (dict);
